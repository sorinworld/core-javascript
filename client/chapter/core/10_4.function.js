...
// 문제 해결 능력이 본인에게 있으면 나 자신을 다시 호출한다.

// -----------------------------------------------------------------------

// pow 함수를 재귀 호출 방식으로 변경
// - 재귀 기반(base) : 재귀를 언제 멈출 것인가? (ex n == 1)
// - 재귀 단계(step) : 재귀 시 반복적으로 처리할 일 (공통적인 처리)
// - 재귀 깊이(depth) : 재귀를 최대 몇 번 수행할 것인가?
// 재귀는 함수가 실행된 곳 부터 이해해야한다.
...
function fibonacci(n){
  ...
  if(n <= 2) return 1; //false 이므로 밑에 식이 실행
...
// 정지되고 쌓인 스택 순서대로 일 처리
// 스택은 아래에서 위로 쌓인다.

// -----------------------------------------------------------------------

// 반복문 기반 알고리즘 vs. 재귀 호출 알고리즘 //성능적으로는 반복문이 좋으나 재귀가 가독성이 좋다.
...
// 메모이제이션(pattern)을 사용한 fibonacci 함수를 작성해보세요.
...
const memoFibo = (n)=>{
  if(n <= 0) return 0; 
  if(n <= 2) return 1;
  if(memoFibo.cache[n]){ //객체의 문법 / 이 함수 안에서만 쓰기위해 / 상수로 쓰면 다음에 또 사용될때 불려갈 수 있어서
