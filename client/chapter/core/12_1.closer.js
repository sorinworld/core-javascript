...
function first(){
  ...
  function second(){
    ...
    let y = 30;
    x + y;
    //x 값 찾아 올라가는거 스코프 체이닝
    //바깥 환경을 지켜보고있다가 본인에게 없는 것을 부모에게서 찾는다
    //secon 함수는 밖에 있는 변수를 가져올 가능성을 열어놓는다..?
  }

  return second //first()를 실행하면 second()함수 전체를 내보낸다. js 함수 = 값
  //second 값까지 같이  실행하려면 first()() =first함수가 second 그자체라서 
  //return second() 하면 함수가 실행되면서 값이 나온다.
  //값만 내뱉으면 의미가 없으니 함수 자체를 내보내는 것 그래야 재사용할 수 있으니까
}

const value = first();
// value를 함수처럼 실행시킬 수있게 된다 value() = 40 =>second의 능력 실행



//클로저를 사용하는 이유 : 전역의 오염을 막기위해
//let count = 0; //전역오염 /함수는 캡슐화 시켜야한다.

function number(){

  let count = 0; //함수 안에 있기때문에 외부에서 찾을 수 없다 (스코프)
  ...
  // return inner
  return() => ++count; //this를 쓰면 문제됌 안쓰는 전제하에 ㄱㅊ
  }



const counter = number(); //카운터 입장에서 이너는 상관없음 필요없음
//함수를 두번 할당하지말고 어디에 변수로 설정해서 할당한다.


//가비지 콜렉터는 함수가 한번 실행되고 나면 수거해간다 / counter는 count에 참조되어 있기때문에 
//가비지 컬렉터가 접근할 수 없다. function number(){}라는 공간을 밀폐시켜 놓은 것과 같다
//안에 있는 함수가 밖으로 나갔을때도 안을 확인할 수 있다. 
